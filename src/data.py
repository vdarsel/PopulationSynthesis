...
def get_category_sizes_pd(X: pd.DataFrame) -> List[int]:
    # XT = X.T.cpu().tolist() if isinstance(X, torch.Tensor) else X.T.tolist()
    return [X[x].nunique() for x in X.columns]
...
class Dataset:
    ...
    @classmethod
    def from_dir(cls, dir_: Union[Path, str], filename: str) -> 'Dataset':
        dir_ = Path(dir_)
        splits = [k for k in ['train', 'test'] if os.path.join(dir_,f'_{k}.'.join(filename.split('.')).exists())]
        ...
        return Dataset(
            {x: data[idx_num] for x in splits} if len(idx_num)>0 else None,
            {x: data[idx_cat] for x in splits} if len(idx_cat)>0 else None,
            info.get('n_classes'),
        )
...
def num_process_nans(dataset: Dataset, policy: Optional[NumNanPolicy]) -> Dataset:
    ...
    if not np.array([x.any().any() for x in nan_masks.values()]).any():  # type: ignore[code]
...
def cat_process_nans(X: ArrayDict, policy: Optional[CatNanPolicy]) -> ArrayDict:
    ...
    if np.array([x.any().any() for x in nan_masks.values()]).any():  # type: ignore[code]
        if policy is None:
            X_new = X
        elif policy == 'drop-rows':
            print(X)
            X_new = X[~pd.isna(X).any(1)]
        ...
        print("No NaNs in categorical features, skipping")
        X_new = X
    return X_new
...
def cat_drop_rare(X: ArrayDict, min_frequency: float) -> ArrayDict:
    ...
    return cat_drop_rare_min_count(X, min_count)
    # X_new = {x: [] for x in X}
...
def cat_drop_rare_min_count(X: ArrayDict, min_count: int) -> ArrayDict:
    assert 0 < min_count
    X_new = {x: X[x].copy() for x in X}
    for column_idx in (X['train'].columns):
        value_counts = X["train"][column_idx].value_counts()
        popular_categories = value_counts[value_counts>=min_count].index
        for part in X_new:
            X_new[part].loc[~X_new[part][column_idx].isin(popular_categories),column_idx] = CAT_RARE_VALUE
            # X_new[part].append(
            #     [
            #         (x if x in popular_categories else CAT_RARE_VALUE)
            #         for x in X[part][:, column_idx].tolist()
            #     ]
            # )
    # print(np.sort(X_new["train"]["iris"].unique()))
    # raise("E")
    return X_new
...
class Transformations:
    ...
    cat_min_count: Optional[int] = None
...
def transform_dataset(
    ...
    if dataset.X_cat is None:
        ...
        if transformations.cat_min_count is not None:
            X_cat = cat_drop_rare_min_count(X_cat, transformations.cat_min_count)
        ...
        if is_num:
            X_num = (
                X_cat
                if X_num is None
                else {x: np.hstack([X_num[x], X_cat[x]]) for x in X_num.keys()}
            )
            X_cat = None
    ...
    print("Final categories (training):", [len(np.unique(dataset.X_cat["train"][:,id])) for id in range(dataset.X_cat["train"].shape[1])])
    return dataset
...
def read_changed_val(path, val_size=0.2):
    path = Path(path)
    X_num_train, X_cat_train, y_train = read_data(path, 'train')
    X_num_val, X_cat_val, y_val = read_data(path, 'val')
