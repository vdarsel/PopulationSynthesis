...
|```--no_training```      |Skips the training phase for DGMs, as their parameters are saved after training.|(Optional flag)      |
...
In the default configuration, the generated population is saved in ```Results>Generated_data>Census_Data_2021_NAME_set_SIZE%_MODEL```, and the scores are saved in ```Results>Generated_data>Census_Data_2021_NAME_set_SIZE%_MODEL>French_Census_Data_2021>scores.csv```. 
...
|-------------------|---------------------------------------------------------------------|---------------------|
|Frequentist        |Transition probabilities are computed from observed frequencies.     |```--MCMC_freq```          |
|Bayesian           |Uses the Bayesian Posterior of a Dirichlet distribution for transition probabilities.|```--MCMC_Baysian```      |
...
|-------------------|---------------------------------------------------------------------|---------------------|-------------------------|
|$\beta$-VAE        |```--beta_VAE```          |```--beta_VAE_dim``` for the layer reference dimension <br>```--beta_VAE_beta``` for the $\beta$ value|[$\beta$-VAE](https://openreview.net/forum?id=Sy2fzU9gl)|
|$\beta$-VAE with continuous pre-embedding        |```--beta_VAE_embedded```      |```--beta_VAE_embedded_dim``` for the layer reference dimension <br>```--beta_VAE_embedded_beta``` for the $\beta$ value |[$\beta$-VAE](https://openreview.net/forum?id=Sy2fzU9gl) <br> [Embedding](https://openreview.net/forum?id=4Ay23yeuz0) |
|TVAE        |```--TVAE```      |```--TVAE_dim``` for the layer reference dimension <br>```--TVAE_beta``` for the $\beta$ value | [TVAE](https://proceedings.neurips.cc/paper_files/paper/2019/file/254ed7d2de3b23ab10936522dd547b78-Paper.pdf)|
...
|Model              |Flag                 | Options                 | Reference |
|-------------------|---------------------------------------------------------------------|---------------------|-------------------------|
|WGAN    |```--WGAN```          |```--WGAN_dim``` for the layer reference dimension |[WGAN](https://proceedings.mlr.press/v70/arjovsky17a.html)|
|WGAN with continuous pre-embedding        |```--WGAN_embedded```      |```--WGAN_embedded_dim``` for the layer reference dimension |[WGAN](https://proceedings.mlr.press/v70/arjovsky17a.html) <br> [Embedding](https://openreview.net/forum?id=4Ay23yeuz0) |
...
|Model              |Flag                 | Options                 | Reference |
|-------------------|---------------------------------------------------------------------|---------------------|-------------------------|
|TabSyn    |```--Diffusion```          |```--Diffusion_dim``` for the layer reference dimension |[TabSyn](https://openreview.net/forum?id=4Ay23yeuz0)|
...
- All parameters can be changed in the configuration files in the ```config``` folder. Some parameters are dependent on the attribute set, so they are defined in ```config/config_variable/NAME.yml```. Some other depends on the training set size and are defined in ```config/config_size/SIZE.yml```.