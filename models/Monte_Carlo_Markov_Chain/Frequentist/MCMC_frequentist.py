...
def gibbs_sampling_one_pass_np_dict(data_np, translation_dict_values, translation_dict_freq, n_cols = -1):
    keep_col = np.array([True for _ in range(n_cols)])
    for i in range(n_cols):
        keep_col_temp = keep_col.copy() 
        keep_col_temp[i] = False
        ...
        data_np[i] = np.random.choice(values,p=freq)
    return data_np
...
def MCMC_frequentist_learn_sample(args):
    
    print("\n\nTraining MCMC with frequentist approach\n\n")
    ...
    for i in tqdm(range(n_cols)):
        vals = (np.unique(opposite_data_tot[:,i]))
        dict_trans = {val: {} for val in vals}
    ...
    for i in tqdm(range(n_cols)):
        ...
        for key_1 in transl[i]:
            ...
            dict_freq[key_1] = dict_count[key_1]/np.sum(dict_count[key_1])
        transl_values.append(dict_val)
        transl_freq.append(dict_freq)
    ...
    print("\nSampling")
    for i in tqdm(range(args.MCMC_frequentist.warm_up+(args.n_generation)*args.MCMC_frequentist.thinning)):
        data = gibbs_sampling_one_pass_np_dict(data,transl_values, transl_freq,n_cols)
        if (i>args.MCMC_frequentist.warm_up):
            if(i%args.MCMC_frequentist.thinning==0):
